...
$(function () {
    createCalendar();
    let url = "http://127.0.0.1:5000/api/getMyItinerary";
    ...
    $.post({
        ...
        success: function (myItinerary) {
            if (myItinerary.length && Array.isArray(myItinerary)) {
                myItinerary[0].itinerary.forEach(function (destination){
                    accordion.append('<h1 class="headerDate">'+ destination.destination+'</h1>');
                            ...
                            destination[date].forEach(function (event) {
                                if(typeof event.event === 'object'){
                                    let htmlString = `
                                        ...
                                        <a class="info" href="${event.event.website}" target="_blank" data-website="${event.event.website}">Website</a>
                                        <a class="info where" data-lat="${event.event.coordinates.lat}" data-lon="${event.event.coordinates.lon}">where I am ?</a><br>`;
