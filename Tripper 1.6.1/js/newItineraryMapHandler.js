    ...
    function filterAttraction(type,secondType, places,map){
        ...
        for (let place of places) {
            ...
            if(place.type === type || place.type === secondType) {
                ...
                placeCard.innerHTML = `<h2 data-name="${place.name}">${place.name}
                                            <input data-hour="" class="hour" type="time" name="orario" onchange="updateDataHour(this)"></h2>
                                       ...
                                       <a data-lat="${place.latitude}" data-lon="${place.longitude}">where I am ?</a>
                                        <script>function updateDataHour(elem){
                                            let nuovoOrario = elem.value;
                                            $(elem).attr("data-hour", nuovoOrario);
