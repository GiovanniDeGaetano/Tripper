...
function initializeDroppable(selector) {
    ...
    $(selector).droppable({
        ...
        drop: function (event, ui) {
            ...
            draggedItem.append('<button class="delete-button delete-style">Delete</button>');
...
function setupPersonalizedStopButton(buttonSelector) {
    $(document).on('click', buttonSelector, function() {
        ...
        $('<input type="time" required>').appendTo(innerContainer);
        ...
        $('<button class="deletePersonalizedStop delete-style">Delete</button>').appendTo(container);
...
function saveItinerary(){
    ...
    for(let i = 0;i < dates.length; i++) {
        ...
        array.sort(function (a, b) {
            // Converti le stringhe "hour" in oggetti Date
